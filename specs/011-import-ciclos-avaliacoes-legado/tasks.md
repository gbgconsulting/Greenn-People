...
- [X] T028 [P] Percorrer `specs/011-import-ciclos-avaliacoes-legado/quickstart.md` completo (C0–C7) e verificar SC-001…SC-010 aplicáveis com samples; smoke opcional com `raw/` **somente** manual em staging (fora do CI)
- [X] T029 [P] Confirmar relatório stdout/`--report-file` usa **amostra mascarada** exclusivamente (máx. 5 por seção; sem nomes/e-mails em massa) em `apps/accounts/services/legacy_import/report.py` (SC-010); inspecionar ausência de dump PII — evidência: `pytest tests/test_import_ciclos_avaliacoes_legado.py -q` cobre T029 (`test_format_ciclos_avaliacoes_report_mascara_e_trunca_max_5`, `test_stdout_e_report_file_iguais_e_mascarados`, `test_mask_solides_id_nunca_emite_completo`, `test_legacy_import_ciclos_nao_loga_linha_completa_backup`, `test_import_ciclos_nao_emite_pii_em_caplog`); `format_ciclos_avaliacoes_report` trunca `_SAMPLE_MAX=5`; stdout **==** `--report-file`; IDs via `mask_solides_id`; e-mails/CPF/RG e nomes de avaliado (`Ana Silva` etc.) ausentes; pacote `cycles/legacy_import` + comando + `report.py` sem `logging`/`print` de linha completa
- [X] T030 [P] Atualizar `data/legado-solides/README.md` (se necessário) com ponteiro ao comando `importar_ciclos_avaliacoes` e ordem segura passo 4→5 — sem expor PII; denylist intacta
- [X] T031 Gate final regressão denylist: `git diff` vs base da feature nos paths denylist = **vazio** (mesmos paths da T019, **incluindo** `cycle.py`) **e** suíte completa verde: `pytest tests/test_stage_machine tests/test_scope tests/test_reject_stage_invariant tests/test_import_ciclos_avaliacoes_legado.py -q`. Migration: **ADITIVA somente Ciclo.solides_id**. Allowlist-only no diff restante. Sem chamada a stage/open/close/approval/calcular no código novo.
