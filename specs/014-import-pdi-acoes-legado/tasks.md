...
- [X] T022 [P] Percorrer `specs/014-import-pdi-acoes-legado/quickstart.md` completo (C0–C10) e verificar SC-001…SC-012 aplicáveis com samples; smoke opcional com `raw/` **somente** manual em staging (fora do CI)
  **DONE 2026-08-19**: Walkthrough C0–C10 com `data/legado-solides/samples/pdi_min.xlsx` (CI **não** leu `raw/`). C0: `git diff development` denylist + `**/migrations/**` = **0 linhas**. C1 SC-006: `test_t016_dry_run_*` (N0/M0). C2 SC-002/003/004: `test_t016_persist_1_pdi_1_acao_*` + `test_t010_c2_*` (1+1, concat `\n\n`, responsável = dono, zero FK ciclo/avaliação no modelo). C3+C4 SC-001: `test_t017_sc001_*` / `test_t013_c3_*` / `test_t013_c4_*` (órfão, ambíguo, `id_vs_nome`, inativo ok, zero User inventado). C5 SC-011/012: `test_t017_c5_*` (`finalizado`→concluído/concluída; `em_andamento`+prazo; conflitos skip; zero `arquivado`; spy `mark_overdue_pdi_actions` não chamado). C6 SC-007: `test_t018_c6_*` (`pdi_`+40 hex, ≠ Nome+Título). C7 SC-005: `test_t018_c7_*` (2ª run delta 0). C8 SC-008: `test_t018_c8_*` (stdout == `--report-file`; máx. 5; sem PII completa). C9: `pytest tests/test_import_pdi_legado.py` → **41 PASS**; `test_t016_suite_usa_somente_samples` + `test_t018_c9_*` (zero path `raw/`). C10 SC-010: `pytest tests/test_stage_machine.py tests/test_scope.py tests/test_reject_stage_invariant.py` + import → **66 PASS** no lote; asserts vs `development` **0 linhas**; `test_t018_c10_*` / ouro denylist. Homologação dump real (~86 linhas, 67/19, SC-009 tempo operador) permanece **manual em staging** (`--dry-run` primeiro).
- [X] T023 [P] Confirmar relatório stdout/`--report-file` usa **amostra mascarada** exclusivamente (máx. 5 por seção; zero dump de título/objetivo/situação completos / nome / e-mail; logs sem linha XLSX crua) em `apps/accounts/services/legacy_import/report.py` (SC-008); evidência nos testes T018; stdout **==** `--report-file`; IDs via `mask_solides_id`; denylist intacta
  **DONE 2026-08-19**: `format_pdi_report` (T006/T023) trunca máx. 5/seção, mascara digest/IDs com `mask_solides_id`, redige título/objetivo/situação/nome/e-mail e motivos livres de órfão; comando emite o **mesmo** texto UTF-8 em stdout e `--report-file` (stderr vazio; sem `logging`/print de linha XLSX). Evidência T018: `test_t018_c8_relatorio_mascarado_stdout_igual_report_file` + T023: `test_t023_*`. `pytest tests/test_import_pdi_legado.py -q` → **46 PASS** (35.88s). Denylist vs HEAD **0 linhas**; `*/migrations/*` **0**.
- [X] T024 [P] Atualizar `data/legado-solides/README.md` (passo 7 / PRD 6.5.6) para o comando fechado `importar_pdi`, `--pdi`, `--dry-run` obrigatório em staging, pré-condição 003+010 (011/013 opcionais) e ponteiro a `samples/pdi_min.xlsx` — sem expor PII; denylist intacta
  **DONE 2026-08-19**: README passo 7 documenta `importar_pdi --pdi` (sample `pdi_min.xlsx` no CI; `raw/` só staging), `--dry-run` obrigatório antes do persist, pré-condição 003+010 (011/013 opcionais), denylist de produto PDI explícita. Sem PII além do inventário já existente (86/67/19).
- [X] T025 Gate final regressão denylist: `git diff` vs base da feature nos paths denylist = **vazio** (mesmos paths da T014) **e** suíte completa verde: `pytest tests/test_stage_machine tests/test_scope tests/test_reject_stage_invariant tests/test_import_pdi_legado.py -q`. **Zero** `*/migrations/*`. Allowlist-only no diff restante. Sem chamada a stage/open/close/approval/`mark_overdue_pdi_actions`/`calcular_aderencia`/`get_visible_users`. Persistência só `full_clean`+`save`. Asserts de stage/scope/reject **não** alterados.
  **DONE 2026-08-19**: base `514ef72` (`development`)→worktree. Denylist **0 bytes** (`stage.py`/`cycle.py`/`approval.py`/`scope.py`/`evaluation.py`/`adherence.py`/`dashboard/urls.py`/`cycles/urls.py`/`pdi/models.py`/`views.py`/`urls.py`/`forms.py`/`overdue.py`/`progress.py`/`tasks.py`/`talent`). `*/migrations/*` **0**. `pytest tests/test_stage_machine.py tests/test_scope.py tests/test_reject_stage_invariant.py tests/test_import_pdi_legado.py -q` → **71 PASS** (50.90s). Asserts stage/scope/reject **intactos** vs base (diff **0**). Diff restante allowlist: `pdi/services/legacy_import/**`, comando `importar_pdi`, `parse_xlsx`/`dates`/`report`, `samples/**`, `tests/test_import_pdi_legado.py`, specs `014-*`, `.specify/feature.json`. Importer/comando **não** chamam `open_cycle`/`close_cycle`/`advance_stage`/`mark_overdue_pdi_actions`/`calculate_pdi_progress`/`calcular_aderencia`/`get_visible_users`/`bulk_create`; persistência só `full_clean()`+`save()`.
