...
- [X] T027 Incluir URLconfs dos apps em `config/urls.py` e expor rotas públicas de accounts + shell de dashboard autenticado mínimo em `apps/dashboard/views.py` (`PersonalDashboardView` em `/`)
...
- [X] T040 [US2] Implementar `normalize_score`, `calcular_nota_final_lider` e `calcular_nota_final_autoavaliacao` em `apps/reviews/services/evaluation.py` conforme `contracts/calculation-contract.md`
- [X] T041 [US2] Implementar `LeaderAssessmentView` + `LeaderAssessmentForm` em `apps/reviews/views.py` / `forms.py` e template `templates/reviews/leader_assessment.html`
...
- [X] T059 [US4] Ao encerrar ciclo, bloquear `advance_stage` (`CycleClosedError`) e marcar avaliações incompletas para indicador de conclusão em `apps/cycles/services/cycle.py`
