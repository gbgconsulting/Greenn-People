...
- [X] T034 [P] Conferir 100% das superfícies com chart (admin, time, estrutura, aderência, `ciclo_list`/`ciclo_detail`, pessoal) contra [contracts/density-history-empty.md](./contracts/density-history-empty.md): teto de densidade, empty honesto, leveza Freeze D, doughnut+centro, `bar_horizontal` ranking, `area` tendência, sem figcaption duplicando label+valor, ~375px sem scroll horizontal do canvas (SC-004 / SC-005 / SC-006)
  **DONE 2026-08-15**: Auditoria Freeze D — superfícies com chart (admin/team/structure/adherence/ciclo_detail/pessoal) OK: Top-N via `coverage_bar_payload`/`top_n_with_others`, empty kinds, pipeline/`cobertura` = `bar_horizontal`, tendência = `area`, doughnut+centro, `min-w-0` + `.dashboard-chart-canvas` (15.5rem). `ciclo_list` sem chart (só agrupamento operacional/arquivo). Partials HTMX sem `_chart_block`. Pessoal sem toggle histórico. **Fix**: `_chart_block` deixa de emitir figcaption para `area` (catálogo 009: legenda Chart.js + tooltip). Docs/contrato alinhados; asserts em `tests/test_dashboard_history_mode.py` (T034).
- [X] T035 [P] Rodar regressão **sem** alterar asserts: `pytest tests/test_stage_machine.py tests/test_scope.py tests/test_reject_stage_invariant.py` (SC-007); diff denylist vazio (`stage.py`, `cycle.py`, `approval.py`, mutators `evaluation.py`, `adherence.py`, `scope.py`, `tasks.py`, `apps/pdi/**`, migrations de nota)
  **DONE 2026-08-15**: SC-007 — `pytest tests/test_stage_machine.py tests/test_scope.py tests/test_reject_stage_invariant.py` → **25 passed**; asserts desses arquivos **não** alterados vs base da feature (`5e3c14e`). Diff denylist vazio vs `5e3c14e`/`development`: `stage.py`, `cycle.py`, `approval.py`, `evaluation.py`, `adherence.py`, `scope.py`, `tasks.py`, `apps/pdi/**`, migrations. Sem hooks `before`/`after_implement`.
- [X] T036 Validar sequência de `specs/012-gerencial-historico-legado/quickstart.md` (gates 0–4) e comandos pytest listados; evidências sugeridas (empty operacional, pipeline do aberto, `area` N≤8, pessoal sem tendência)
  **DONE 2026-08-15**: Quickstart gates 0–4 validados.
  - **Gate 0**: denylist diff vazio vs `5e3c14e`/`development` (`stage.py`/`cycle.py`/`approval.py`/`scope.py`/`evaluation.py`/`adherence.py`/`tasks.py`/`apps/pdi/**`/migrations); regressão domínio coberta na suite.
  - **Gates 1–3 (evidências via pytest)**: `test_admin_sem_ciclo_aberto_empty_operacional` (empty operacional); `test_admin_com_ciclo_aberto_kpis_e_pipeline_so_desse_ciclo` (pipeline do aberto); `test_admin_visao_historico_area_cap_n_e_kpis_janela` (`area` N≤8); `test_personal_must_not_honor_visao_historico` + `test_t034_pessoal_sem_toggle_historico` (pessoal sem tendência) — **5/5 passed**.
  - **Gate 4**: Freeze D em `docs/design-system.md`; Chart.js **4.5.1** nas superfícies com chart; `base.html` sem Chart.js global; partials HTMX sem `_chart_block`; sem rota `/historico/`.
  - **Suite quickstart**: `pytest tests/test_dashboard_operational_default.py tests/test_dashboard_history_mode.py tests/test_chart_payloads.py tests/test_structure_coverage.py tests/test_ciclo_detail.py tests/test_stage_machine.py tests/test_scope.py tests/test_reject_stage_invariant.py` → **137 passed**. Sem hooks `before`/`after_implement`.
- [X] T037 Confirmar `static/js/dashboard_charts.js`: Chart.js **4.5.1**, init `DOMContentLoaded`, **sem** `htmx:afterSwap`, **sem** plugin npm; `base.html` sem Chart.js global; partials HTMX ainda sem `_chart_block` (FR-017)
  **DONE 2026-08-15**: Auditoria FR-017 confirmada — sem mudança de código de produção.
  - `dashboard_charts.js`: init via `DOMContentLoaded` → `initAll`; **sem** `htmx:afterSwap`; plugins inline (`afterDraw` / `afterDatasetsDraw`); **sem** `require`/`import`/plugin npm.
  - Superfícies com chart (`admin`/`team`/`structure`/`adherence`/`personal`/`ciclo_detail`): CDN `chart.js@4.5.1` + `dashboard_charts.js` só em `extra_js`.
  - `base.html`: sem Chart.js / sem `dashboard_charts` (só HTMX global).
  - Partials HTMX (`team_list_partial` / `adherence_list_partial` / `ciclo_list_partial`): sem `_chart_block` / sem canvas.
  - Asserts: `test_t037_dashboard_charts_init_sem_htmx_after_swap` + `test_t037_base_sem_chartjs_global_e_partials_sem_chart_block` → **2 passed**. Sem hooks `before`/`after_implement`.
