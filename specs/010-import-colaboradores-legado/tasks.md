...
- [X] T029 [P] [US5] Criar fixtures XLSX anonimizadas mínimas (`colaboradores_min.xlsx`, `avaliacoes_crosswalk_min.xlsx`) em `data/legado-solides/samples/` — subset: ativos, demitidos, sem superior, crosswalk parcial, datas serial+ISO; **sem PII real**; denylist intacta — evidência 2026-08-13: OOXML `sheet1`; 8 colaboradores (`@example.com`, CPF sentinela ignorado); demissão serial `45446.0` (Carla) + ISO `2024-06-01` (Diego); sem superior (Gestor Alpha/Carla/Fernanda/Nome Ambiguo/Elena); crosswalk parcial (Fernanda sem match) + ambíguo (`Nome Ambiguo` 900/901); parse+crosswalk OK; denylist 0 linhas.
- [X] T030 [P] [US5] Criar `tests/test_import_colaboradores_legado.py` com testes de dry-run (zero writes) e idempotência (2ª execução delta duplicatas=0) usando **somente** `data/legado-solides/samples/` (SC-008; **proibido** `raw/`; denylist intacta) — evidência 2026-08-13: `pytest tests/test_import_colaboradores_legado.py -q` **5 PASS**; dry-run `modo: dry-run` + `usuarios_criados: 7` projetados e User/Area/Cargo inalterados (SC-006); CLI `--dry-run` exit 0; 1ª persist `usuarios_criados: 7` / 2ª `usuarios_criados: 0` `usuarios_atualizados: 0` `usuarios_inalterados: 7` (delta e-mail = 0, SC-007); dry-run pós-persist projeta inalterados sem writes; paths só `data/legado-solides/samples/` (SC-008).
- [X] T031 [P] [US5] Adicionar testes de demitidos inativos, ordem preferência e-mail, Area/Cargo resolve e crosswalk parcial/ambíguo em `tests/test_import_colaboradores_legado.py` (allowlist; denylist intacta) — evidência 2026-08-13: `pytest tests/test_import_colaboradores_legado.py -q` **12 PASS** (5 T030 + 7 T031); serial `45446.0` → `2024-06-03` e ISO `2024-06-01` inativam Carla/Diego (`demitidos_inativos: 2`, SC-003); `resolve_email` empresarial→`E-mail`→pessoal + fixture Ana/Bruno/Diego; Area 3 / Cargo 4 com `solides_id` + reuso `Engenharia Fixture`; crosswalk 5 únicos / Fernanda null / Nome Ambiguo `crosswalk_ambiguo` ids 900,901 (SC-005); paths só `samples/` (SC-008); denylist intacta.
- [X] T032 [P] [US5] Adicionar testes de hierarquia (`line_manager` resolvível), ciclo reportado (vínculo não aplicado) e arquivo inválido (exit 1, DB inalterado) em `tests/test_import_colaboradores_legado.py` (denylist intacta) — evidência 2026-08-13: `pytest tests/test_import_colaboradores_legado.py -q` **17 PASS** (12 T030–T031 + 5 T032); C3 `Superior direto id` 100 → Ana/Bruno/Diego `line_manager` = Gestor Alpha (`gestores_vinculados: 3`, `sem_gestor: 4`); superior `99999` → `gestor_nao_resolvido` e vínculo anterior intacto; ciclo artificial Gestor Alpha→Ana → `ciclos_hierarquia` 1 (`ciclo_detectado`) e `line_manager` do gestor permanece null; C4.3 arquivo ausente e OOXML corrompido → `CommandError.returncode == 1` + `LegacyParseError`, User/Area/Cargo inalterados; paths só `samples/` + `tmp_path` (SC-008); denylist intacta.
- [X] T033 [US5] Adicionar teste migration reversível (`solides_id` forward/backward preserva seed) e assert diff denylist vazio pós-import em `tests/test_import_colaboradores_legado.py` conforme `contracts/migration-safety.md` §4 (denylist intacta) — evidência 2026-08-13: `pytest tests/test_import_colaboradores_legado.py -q` **19 PASS** (17 T030–T032 + 2 T033); C1 round-trip `0002/0004/0003/0007/0002_add_solides_id` → previous e de volta: coluna some/volta, seed User/Cargo/Competencia/Avaliacao/PDI (email/nome/etapa) intacto, `solides_id` descartado no reverse, unique non-null → `IntegrityError`; pós-import hashes denylist inalterados + `git diff HEAD` e `git diff 2967bf9 HEAD` = **0 linhas** (`stage.py`/`cycle.py`/`approval.py`/`scope.py`/`evaluation.py`/`adherence.py`); paths só `samples/` (SC-008).
- [X] T034 [US5] Validar US5 via `specs/010-import-colaboradores-legado/quickstart.md` C5 + C6 (pytest verde; relatório sem CPF; login bloqueado sem reset); confirmar CI não lê `raw/` — evidência 2026-08-13: `pytest tests/test_import_colaboradores_legado.py -q` **23 PASS** (19 T030–T033 + 4 T034); C5.1 suíte `test_*.py` sem path `data/legado-solides/raw` nem dumps `backup_*_20260624` (SC-008); C5.2 stdout `Amostra (mascarada` + `***@example.com`, e-mails completos e CPF sentinela `000.000.000-00` ausentes (SC-009); C5.3 `CustomUser`/colunas sem cpf/rg/endereço/telefone/banco; C6 Ana importada `has_usable_password() is False` → `authenticate`/`Client.login`/POST `accounts:login` falham, após `set_password` autenticam (`_auth_user_id` na sessão); `.gitignore`/`.dockerignore` excluem `data/legado-solides/raw`; denylist `git diff 2967bf9` + working tree = **0 linhas**.
...
- [X] T035 [P] Percorrer `specs/010-import-colaboradores-legado/quickstart.md` completo (C1–C6) e verificar SC-001…SC-010 aplicáveis; smoke opcional com `raw/` **somente** manual em staging (fora do CI) — evidência 2026-08-13: `pytest tests/test_import_colaboradores_legado.py -q` **25 PASS** (23 T030–T034 + `test_ignore_files_excluem_raw` + `test_quickstart_c1_c6_end_to_end_samples`, 1.71s o walkthrough / 5.80s a suíte); C1/SC-001 cinco entidades `solides_id` null + unique `IntegrityError` + seed/login/ciclo `ENCERRADO`/`INPUT_METAS` intactos; C2 dry-run `modo: dry-run` `usuarios_criados: 7` zero writes (SC-006) + persist `usuarios_criados: 7` `nao_importaveis: 1` (7/7 e-mails válidos, análogo SC-002) + Carla/Diego `is_active=False` / Ana+Gestor ativos (SC-003) + `email_confirmado_em` + áreas fixture; C3 `solides_id_preenchidos: 5` Fernanda/Ambiguo null (SC-005) + `gestores_vinculados: 3` `sem_gestor: 4` (análogo SC-004); C4 2ª execução `usuarios_criados: 0` `usuarios_inalterados: 7` (SC-007) + `--report-file` = stdout; C5 `.gitignore`/`.dockerignore` excluem `data/legado-solides/raw` (SC-008) + amostra mascarada sem CPF (SC-009); C6 Ana `has_usable_password() is False`; SC-010 persist+relatório **< 2 s** (≪ 5 min / 2 min); KPIs de dump (~325 / ~260 / ~187) **não** medidos aqui — smoke `raw/` opcional só em staging; denylist `git diff 2967bf9` + working tree = **0 linhas**.
- [X] T036 [P] Confirmar relatório stdout/`--report-file` usa **amostra mascarada** exclusivamente (`mask_email`/`mask_pii` em `report.py`); inspecionar ausência de CPF/RG/endereço em logs e registros importados (SC-009) — evidência 2026-08-13: `pytest tests/test_import_colaboradores_legado.py -q` **32 PASS** (25 T030–T035 + 7 T036); `format_report` amostra passa por `mask_email`/`mask_pii` (e-mail `a***@example.com`, CPF/RG formatados `***`); CLI stdout **==** `--report-file` (UTF-8) sem e-mails completos nem sentinela `000.000.000-00`; parse ignora coluna `CPF` da fixture (`ColaboradorRow`/allowlist sem cpf/rg/endereço); User/Area/Cargo persistidos sem valores PII; pacote `legacy_import` + comando sem `logging`/`print` de linha completa (R14); `caplog` DEBUG sem CPF/e-mail completo; denylist `git diff 2967bf9` + working tree = **0 linhas** (`stage.py`/`cycle.py`/`approval.py`/`scope.py`/`evaluation.py`/`adherence.py`).
- [X] T037 Gate final regressão denylist: `git diff main --` paths denylist = **vazio** (mesmos paths da T024) **e** suíte completa verde: `pytest tests/test_stage_machine tests/test_scope tests/test_reject_stage_invariant tests/test_import_colaboradores_legado.py -q`. Migrations: **ADITIVA somente solides_id**. Allowlist-only no diff restante. — evidência 2026-08-13: base efetiva da feature `2967bf9` (merge 009)→HEAD + working tree — denylist **0 linhas** (`stage.py`/`cycle.py`/`approval.py`/`scope.py`/`evaluation.py`/`adherence.py`); `git diff main` é ruidoso (`main` = commit inicial sem o app; 6 arquivos / 911 inserções = criação histórica, não mutação desta fatia). pytest `test_stage_machine`+`test_scope`+`test_reject_stage_invariant`+`test_import_colaboradores_legado` **57 PASS** (22.04s). Models **somente +solides_id**. Migrations **somente AddField solides_id** (`accounts/0002`, `organization/0004`, `competencies/0003`, `reviews/0007`, `pdi/0002`). Diff restante = allowlist (`solides_id`, `legacy_import/**`, comando + package inits, `requirements.txt`+openpyxl, `samples/**`, `specs/010-*`, README operacional, ignore `raw/` PII); **exceção operacional** `docker-compose.yml` + celerybeat no `.gitignore` (schedule/pid em `/tmp`, sem domínio) e `.specify/feature.json` (ponteiro 010).
